...
from scripts.eval_metrics import (  # noqa: E402
    LOW_FILL, card_metrics, invented_numbers, pattern_match, table_spill, text_overflows, word_breaks,
)
...
def write_summary(results: dict[str, Any], path: Path) -> None:
    ...
    for key in ("runs_passed", "runs_errored", "slide_count_off", "compiled_pct", "first_pass_pct", "mean_retries",
                ...
                "table_spill_slides", "table_spill_px", "invented_numbers", "auto_fixes_per_slide",
    ...
    lines += ["", f"Fill = card content height ÷ inner height (1.0 = no dead space); low fill < {LOW_FILL}. "
              ...
              "lines more than its box; table spill = table rows past their frame (slides, px); invented numbers = numbers on slides that are not in the brief; "
