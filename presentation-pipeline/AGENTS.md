...
- **Next**: one 12-case gate run on the test device (`python -m scripts.eval_run --label gate-phase-5 --repeat 1 --bundle`) — the first broad measurement and the baseline for Phase 2; then **Phase 2** (routing + planner fixes) in a new session, branch `phase-2-routing` (Phase 2 building needs no key and can start while the gate runs). Unit tests: 424 pass, same 4 pre-existing failures.
