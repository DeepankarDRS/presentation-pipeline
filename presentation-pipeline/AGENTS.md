...
- **Eval `tables-check`** (2026-09-24, `docs/eval/tables-check/`: gj-h1 + 5 table cases): gj-h1 first-pass 92.9%, table spill 187 → 77 px vs phase-1, empty cells 0, golden match 0.547 (best so far). Open table findings (roadmap): two type sizes when two tables share a slide, tables top-aligned in cards stretched by a chart neighbour, touching cells. Non-table: generator writes `<B>` markup inside TimelineItem attributes (shown literally).
- **Next**: **Phase 2** (routing + planner fixes) in a new session, branch `phase-2-routing`; the full 12-case gate (`python -m scripts.eval_run --label gate-phase-5 --repeat 1 --bundle`) still has to run once on `feat/golden-reference-grounding` before Phase 2 changes merge — it is Phase 2's baseline. Unit tests: 425 pass, same 4 pre-existing failures.
