...
- **Next**: **Phase 2** (routing + planner fixes) in a new session, branch `phase-2-routing`; Phase 2's baseline is `docs/eval/tables-check/` (gj-h1 + the 5 table gate cases, already run — do not ask for them again); only the 6 never-run gate cases remain, run once on `feat/golden-reference-grounding` before Phase 2 merges, same label so they merge into that folder: `python -m scripts.eval_run gate-deck-cheffin-audit gate-deck-xtsy-qcomm gate-deck-agency-takeover eval-categorized-list-routing eval-chart-vs-kpi-disambiguation kpi-row --repeat 1 --label tables-check --bundle`. Unit tests: 428 pass, same 4 pre-existing failures.
