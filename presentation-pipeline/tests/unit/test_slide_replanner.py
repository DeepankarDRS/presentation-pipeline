...
@patch("src.agents.slide_replanner.planner_node")
def test_tier0_no_new_kind_skips_replan(mock_planner_node):
    plan = _slide_plan()
    updated_plan, contract = resolve_slide_plan(plan, "make the title bigger", _THEME_INFO, "run-1")

    mock_planner_node.assert_not_called()
    assert updated_plan == plan
    assert "Slide" in contract["allowed_nodes"]


@patch("src.agents.slide_replanner.planner_node")
def test_tier0_removal_feedback_skips_replan(mock_planner_node):
...
@patch("src.agents.slide_replanner.planner_node")
def test_tier2_new_kind_triggers_replan(mock_planner_node):
    plan = _slide_plan()
    replanned_slide = {
        "slide_index": 0,  # planner_node always re-enumerates from 0
        "slide_type": "data",
    ...
    mock_planner_node.return_value = {"core_hook": "", "slide_plans": [replanned_slide]}

    updated_plan, contract = resolve_slide_plan(plan, "add a KPI row", _THEME_INFO, "run-1")

    mock_planner_node.assert_called_once()
...
@patch("src.agents.slide_replanner.planner_node")
def test_tier2_passes_prior_plan_and_feedback_to_state(mock_planner_node):
    plan = _slide_plan()
    mock_planner_node.return_value = {"core_hook": "", "slide_plans": [dict(plan, slide_index=0)]}

    resolve_slide_plan(plan, "add a chart", _THEME_INFO, "run-42")

    state_arg = mock_planner_node.call_args[0][0]
    assert state_arg["run_id"] == "run-42"
    assert state_arg["refine_feedback"] == "add a chart"
    assert state_arg["prior_plan"]["slides"] == [plan]
